...
def get_versions(conn):
    ...
    versions["db_version"] = conn.execute(text(f"SELECT {db_version};")).fetchone()[0] if db_version else ""
    versions["geos_version"] = conn.execute(text(f"SELECT {geos_version};")).fetchone()[0] if geos_version else ""
    versions["proj_version"] = conn.execute(text(f"SELECT {proj_version};")).fetchone()[0] if proj_version else ""
    versions["proj_path"] = conn.execute(text(f"SELECT {proj_path};")).fetchone()[0] if proj_path else ""
    try:
...
def print_versions(versions):
    ...
    if versions["db_version"]:
        print(f"db_version: ", versions["db_version"])
